...
/* We assume in this file that OBJECTIVE is the same as long double */

long double compute_hv (mxArray* f, FRONT *buffer)
{
  size_t i, j;           /* loop indices */
  size_t nb_points;      /* number of points */
  size_t nb_objectives;  /* number of objectives */
  long double *data;          /* pointer to input data */
  long double hv, t;          /* hypervolume */
                            ...
                            mxArray** sign, mxArray** xmin, mxArray** xmax)
{
  ...
  long double t;              /* aux variables */
  long double *data;          /* pointer to input data */
  RLIST* Rlist;          /* list of hyper-rectangles */
  long double *sign_data, *xmin_data, *xmax_data;
  ...
  else /* two ore more objectives */
    {
      ...
      *xmax = mxCreateDoubleMatrix (Rlist->size, nb_objectives, mxREAL);
      ...
      for (i = 0; i < Rlist->size; i++)
        {
          sign_data[i] = (long double) Rlist->sign[i];
          for (j = 0; j < nb_objectives; j++)
            {
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  long double *hv;             /* computed hyper-volumes */
  ...
  if (stk_is_realmatrix (Y_IN))
    ...
    {
      mexErrMsgTxt ("Incorrect type for argin #1: cell or long double expected.");
