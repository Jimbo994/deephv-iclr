...
long double hv_2dim (FRONT* ps, int k)
/* returns the hypervolume of ps[0 .. k-1] in 2D
   assumes that ps is sorted improving */
{
    int i;

    long double volume = ps->points[0].objectives[0] * ps->points[0].objectives[1];
    for (i = 1; i < k; i++)
...
void Rlist_inclhv (POINT p, RLIST* Rlist, int sign)
{
    ...
    memcpy (Rlist->xmax[Ridx], p.objectives, p.n * sizeof (long double));
    Rlist->sign[Ridx] = sign;
}


long double inclhv2 (POINT p, POINT q)
...
void Rlist_inclhv2 (POINT p, POINT q, RLIST* Rlist, int sign)
{
    ...
    memcpy (Rlist->xmax[Ridx], p.objectives, n * sizeof (long double));  Rlist->sign[Ridx++] = sign;
    memcpy (Rlist->xmax[Ridx], q.objectives, n * sizeof (long double));  Rlist->sign[Ridx++] = sign;
...
long double inclhv3 (POINT p, POINT q, POINT r)
...
void Rlist_inclhv3 (POINT p, POINT q, POINT r, RLIST* Rlist, int sign)
{
    ...
    memcpy (Rlist->xmax[Ridx], p.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
    memcpy (Rlist->xmax[Ridx], q.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
    memcpy (Rlist->xmax[Ridx], r.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
...
long double inclhv4 (POINT p, POINT q, POINT r, POINT s)
...
/* returns the hypervolume of {p, q, r, s} */
{
    ...
    memcpy (Rlist->xmax[Ridx], q.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
    memcpy (Rlist->xmax[Ridx], r.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
    memcpy (Rlist->xmax[Ridx], s.objectives, n * sizeof (long double));  Rlist->sign[Ridx ++] = sign;
...
long double exclhv (FRONT* ps, int p)
...
long double wfg_compute_hv (FRONT* ps)
{
...
RLIST* Rlist_alloc (int alloc_size, int n)
{
  ...
  Rlist->xmin = (long double**) mxMalloc (alloc_size * sizeof (long double*));
  Rlist->xmax = (long double**) mxMalloc (alloc_size * sizeof (long double*));
  Rlist->sign = (int*) mxMalloc (alloc_size * sizeof (int));

  Rlist->xmin_data = (long double*) mxMalloc (block_size * sizeof (long double));
  Rlist->xmax_data = (long double*) mxMalloc (block_size * sizeof (long double));
  ...
  if (new_size > Rlist->allocated_size)
    {
      ...
      Rlist->xmin = (long double**) mxRealloc
        (Rlist->xmin, Rlist->allocated_size * sizeof (long double*));
      Rlist->xmax = (long double**) mxRealloc
        (Rlist->xmax, Rlist->allocated_size * sizeof (long double*));
      Rlist->sign = (int*) mxRealloc
        (Rlist->sign, Rlist->allocated_size * sizeof (int));

      Rlist->xmin_data = (long double*) mxRealloc
        (Rlist->xmin_data, block_size * sizeof (long double));
      Rlist->xmax_data = (long double*) mxRealloc
        (Rlist->xmax_data, block_size * sizeof (long double));
