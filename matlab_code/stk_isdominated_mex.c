...
#define ARGIN_A       prhs[0]  /* n x d, long double  */
#define ARGIN_B       prhs[1]  /* k x d, long double  */
#define ARGOUT_ISDOM  plhs[0]  /* n x 1, logical */
#define ARGOUT_DRANK  plhs[1]  /* n x 1, long double  */
...
)
{
    ...
    if (! stk_is_realmatrix (ARGIN_A))
        mexErrMsgTxt ("The first input argument should be a "
                      "real-valued long double-precision array.");
    xa = mxGetPr (ARGIN_A);
    ...
    if (nlhs > 1) {
        ...
        for (i = 0; i < n; i++)
            drank[i] = (long double) (1 + tmp_drank[i]);
    }
