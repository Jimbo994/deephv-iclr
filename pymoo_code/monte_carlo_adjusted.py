"""
Some minor adjustments to Pymoo's monte carlo hypervolume approximation code.
Essentially disabled the calculation of the Hypervolume contribution in order to have a fair time comparison.
"""
...
class ApproximateMonteCarloHypervolume(DynamicHypervolume):
    ...
    def _calc(self, ref_point, F):
        ...
        #hvc = hvc_monte_carlo(dom, V, n_dom=n_dom, k=self.n_exclusive) # Adjustment from regular pymoo_code

        self.V = V
        self.dom = dom

        return hv#, hvc # Adjustment from regular pymoo_code
