...
from custom_acquisition_functions import UpperConfidenceBoundBaselineDeepHVBatched, UpperConfidenceBoundBaselineDeepHVBatchedPadded
...
def optimize_hvi_and_get_observation(model, train_x, train_obj, hv_model):
    ...
    if shape[0] == 0 or train_obj_shifted.sum() == torch.tensor(0, **tkwargs):
        ...
        # first pad with 1s
        # if MODE == 'all':
        #     train_obj_shifted = pad(train_obj_shifted, (0, 10-shape[1], 0, 0), "constant", 1)
    ...
    if MODE == 'all':
        UCB = UpperConfidenceBoundBaselineDeepHVBatchedPadded(model, hv_model, volume, train_obj, prob.ref_point, beta=0.3)
    else:
        UCB = UpperConfidenceBoundBaselineDeepHVBatched(model, hv_model, volume, train_obj, prob.ref_point, beta=0.3)
...
NUM_RESTARTS = 10
RAW_SAMPLES = 64
# If using a model trained on all dimensions.
MODE = 'all'

master_path = "../botorch_results/baseline-deephv-allmodel128/"
    ...
    for dim in dims:
        ...
        for trial in range(1, N_TRIALS + 1):
            ...
            for iteration in range(1, N_BATCH + 1):
                ...
                if verbose:
                    print(
                        f"\nBatch {iteration:>2}: Hypervolume (baseline-deephv) = "
                        f"({hvs[-1]:>4.2f}), "
